package command // import "github.com/NYTimes/Archivehandler"
...
const (
	...
	//but consider Google Cloud Function charged network bandwidth Price/GB $0.12 , we should make it as small as possible
	DefaultMinSize = 128
)
...
// ArchiveResponseWriter provides an http.ResponseWriter interface, which gzips
...
//ArchiveResponseWriterWithCloseNotify handler
type ArchiveResponseWriterWithCloseNotify struct {
	*ArchiveResponseWriter
}

//CloseNotify handler
func (w ArchiveResponseWriterWithCloseNotify) CloseNotify() <-chan bool {
	return w.ResponseWriter.(http.CloseNotifier).CloseNotify()
}

// Write appends data to the gzip writer.
func (w *ArchiveResponseWriter) Write(b []byte) (int, error) {
...
func (w *ArchiveResponseWriter) startGzip() error {
	// Set the GZIP header.
...
func (w *ArchiveResponseWriter) startPlain() error {
	if w.code != 0 {
		w.ResponseWriter.WriteHeader(w.code)
		// Ensure that no other WriteHeader's happen
		w.code = 0
...
func (w *ArchiveResponseWriter) WriteHeader(code int) {
	if w.code == 0 {
		w.code = code
	}
}
...
func (w *ArchiveResponseWriter) init() {
...
func (w *ArchiveResponseWriter) Close() error {
	if w.ignore {
		return nil
	}
	...
	if w.gw == nil {
		...
		if err != nil {
			err = fmt.Errorf("Archivehandler: write to regular responseWriter at close gets error: %q", err.Error())
		}
		return err
	}
...
// http.ResponseWriter if it is an http.Flusher. This makes ArchiveResponseWriter
// an http.Flusher.
...
var _ http.Hijacker = &ArchiveResponseWriter{}
...
func NewGzipLevelAndMinSize(level, minSize int) (func(http.Handler) http.Handler, error) {
	return ArchiveHandlerWithOpts(CompressionLevel(level), MinSize(minSize))
}

// ArchiveHandlerWithOpts handler
//
func ArchiveHandlerWithOpts(opts ...Option) (func(http.Handler) http.Handler, error) {
	c := &config{
		...
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if acceptsGzip(r) {
				...
				if _, ok := w.(http.CloseNotifier); ok {
					gwcn := ArchiveResponseWriterWithCloseNotify{gw}
					h.ServeHTTP(gwcn, r)
				} else {
					h.ServeHTTP(gw, r)
				}
...
// ArchiveHandler wraps an HTTP handler, to transparently gzip the response body if
// the client supports it (via the Accept-Encoding header). This will compress at
// the default compression level.
func ArchiveHandler(h http.Handler) http.Handler {
...
//Push handler
func (w *ArchiveResponseWriter) Push(target string, opts *http.PushOptions) error {
