...
func TestNewGzipLevelHandler(t *testing.T) {
	...
	for lvl := gzip.BestSpeed; lvl <= gzip.BestCompression; lvl++ {
		...
		if !assert.Nil(t, err, "NewGzipLevelHandler returned error for level:", lvl) {
			continue
		}
...
func TestArchiveHandlerContentLength(t *testing.T) {
	...
	for num, test := range tests {
		srv.Handler = ArchiveHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if test.bodyLen > 0 {
...
func TestDontWriteWhenNotWrittenTo(t *testing.T) {
	...
	handler0 := ArchiveHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	}))
...
func TestContentTypes(t *testing.T) {
	for _, tt := range contentTypeTests {
		...
		wrapper, err := ArchiveHandlerWithOpts(ContentTypes(tt.acceptedContentTypes))
		if !assert.Nil(t, err, "NewArchiveHandlerWithOpts returned error", tt.name) {
			continue
		}
...
func BenchmarkArchiveHandler_S2k(b *testing.B)   { benchmark(b, false, 2048) }
func BenchmarkArchiveHandler_S20k(b *testing.B)  { benchmark(b, false, 20480) }
func BenchmarkArchiveHandler_S100k(b *testing.B) { benchmark(b, false, 102400) }
func BenchmarkArchiveHandler_P2k(b *testing.B)   { benchmark(b, true, 2048) }
func BenchmarkArchiveHandler_P20k(b *testing.B)  { benchmark(b, true, 20480) }
func BenchmarkArchiveHandler_P100k(b *testing.B) { benchmark(b, true, 102400) }
...
func newTestHandler(body string) http.Handler {
	return ArchiveHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/gzipped":
